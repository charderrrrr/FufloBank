// PaymentController - API контроллер для обработки платежей.
// Обрабатывает переводы между пользователями по номеру телефона.
// Проверяет лимиты, при необходимости запрашивает подтверждение, списывает средства со счета отправителя и зачисляет получателю.
